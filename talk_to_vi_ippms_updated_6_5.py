...
#  Auth model (per-user)
#  ---------------------
#  The login page asks for email + employee id + password.
#    - password  -> gates access to THIS app (checked app-side, below).
#    - email+eid -> pushed to the MCP `ig_login` tool to mint that user's
#                   Instant Graph token. Every downstream tool call carries a
#                   per-user session_key (the email), so tokens are isolated.
...
# ── App-level authentication (password gate) ─────────────────────────────────
# email+eid go to the MCP for the IG token; the PASSWORD only gates this app.
# The IG API itself does not use a password. Configure app credentials one of:
#   VI_APP_USERS_JSON : JSON like {"first.last@vodafoneidea.com": "<sha256 hex>"}
#   VI_APP_USERS_FILE : path to a JSON file with the same mapping
# where the hash is sha256("<eid>:<password>") lower-hex. If neither is set and
# VI_ALLOW_ANY_VODAFONE=1, any @vodafoneidea.com email is accepted with any
# non-empty password (DEV ONLY — insecure; logs a warning).
APP_ALLOW_DOMAIN     = os.environ.get("VI_APP_ALLOW_DOMAIN", "vodafoneidea.com").lower()
ALLOW_ANY_VODAFONE   = os.environ.get("VI_ALLOW_ANY_VODAFONE", "1") == "1"
...
DB_PASSWORD = os.environ.get("VI_DB_PASSWORD", os.environ.get("IG_DB_PASSWORD", "ig_app_user_Qwer!234"))
...
# ══════════════════════════════════════════════════════════════════════════════
#  SECTION 2 — APP AUTH (password gate) + SESSION TOKENS
# ══════════════════════════════════════════════════════════════════════════════
...
def _load_app_users() -> Dict[str, str]:
    raw = os.environ.get("VI_APP_USERS_JSON", "").strip()
    if not raw:
        path = os.environ.get("VI_APP_USERS_FILE", "").strip()
        if path and os.path.exists(path):
            try:
                raw = open(path, "r", encoding="utf-8").read()
            except Exception as exc:
                log.error("Could not read VI_APP_USERS_FILE: %s", exc)
                raw = ""
    if not raw:
        return {}
    try:
        return {k.strip().lower(): v.strip().lower() for k, v in json.loads(raw).items()}
    except Exception as exc:
        log.error("VI_APP_USERS_JSON/FILE is not valid JSON: %s", exc)
        return {}


APP_USERS = _load_app_users()
if not APP_USERS and ALLOW_ANY_VODAFONE:
    log.warning("No app credential store configured and VI_ALLOW_ANY_VODAFONE=1 — "
                "accepting any @%s email with any non-empty password (DEV MODE, insecure).",
                APP_ALLOW_DOMAIN)


def app_authenticate(email: str, eid: str, password: str) -> Tuple[bool, str]:
    """Validate app-level credentials (the password gate). Returns (ok, message)."""
    email = (email or "").strip().lower()
    eid = (eid or "").strip()
    password = password or ""
    # (removed a log line that printed the password length — Appendix B leak.)
    if not email or not eid or not password:
        return False, "Email, Employee ID and password are all required."
    if APP_ALLOW_DOMAIN and not email.endswith("@" + APP_ALLOW_DOMAIN):
        return False, f"Only @{APP_ALLOW_DOMAIN} accounts are permitted."

    if APP_USERS:
        expected = APP_USERS.get(email)
        if not expected:
            return False, "This email is not provisioned for the app."
        if _sha256(f"{eid}:{password}") != expected and _sha256(password) != expected:
            return False, "Incorrect password or employee id."
        return True, "ok"

    if ALLOW_ANY_VODAFONE:
        return True, "ok"
    return False, "No credentials are provisioned. Contact the administrator."
...
CIRCLE_TOKENS: Dict[str, Dict[str, Any]] = {
    "GUJARAT":        {"aliases": ["guj", "gujarat", "gj"],        "tokens": ["GJ", "GJW", "GJAHD", "GJW-", "AHD"]},
...
def login_page(error: Optional[str] = None):
    return html.Div(className="login-wrap", children=[
        html.Div(className="login-card", children=[
            ...
            dcc.Input(id="login-eid", type="text", className="inp", placeholder="e.g. 22013296", n_submit=0),
            html.Label("Password", className="lbl"),
            dcc.Input(id="login-password", type="password", className="inp",
                      placeholder="Your app password", n_submit=0),
...
def do_login(_n, _s1, _s2, _s3, email, eid, password):
    ok, msg = app_authenticate(email or "", eid or "", password or "")
    if not ok:
        return no_update, html.Div(msg, className="alert-err")
    # App password accepted — now establish this user's Instant Graph token.
    try:
        ig_login_via_mcp(email.strip().lower(), eid.strip())
    except MCPError as exc:
        return no_update, html.Div(f"Instant Graph login failed: {exc}", className="alert-err")
    token = session_create(email.strip().lower(), eid.strip())
    return {"token": token, "email": email.strip().lower()}, None
...
def on_feedback(_up, _dn, feedback, msgs, auth):
    ...
    return feedback, render_stream(msgs or [], feedback, False)
